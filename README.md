...
### 5. 🔄 完整的项目与页面生命周期钩子 (Comprehensive Project & Document Lifecycle Hooks)

DNOTE 提供了一套完整的项目级和页面级生命周期脚本钩子，让笔记系统的自动化处理逻辑能紧密配合用户的使用阶段：
...
*   **页面/文档级交互生命周期 (Document-level Lifecycle)**：支持在 Markdown 文档的 YAML Frontmatter 中声明运行脚本标签（如 `run: script.py`）或反应式插值表达式（如 `{{ ... }}`）。在编辑器中打开、保存、或移动光标时，DNOTE 会自动将实时的编辑器上下文环境变量（当前文件 `DNOTE_ACTIVE_FILE`、项目根目录 `DNOTE_PROJECT_PATH`、光标行/列数、选中文本等）注入到子进程环境中运行，脚本计算完成后通过血液事件即时流转并动态刷新编辑器状态，实现高度互动的页面生命周期。
