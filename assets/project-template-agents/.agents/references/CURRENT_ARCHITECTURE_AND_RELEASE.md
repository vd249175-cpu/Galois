...
- App-level preferences, shortcuts, layout, logs, and side-loaded extensions
  live under `~/Documents/Galois/`.
...
should use the writable user extension directory at
...
and user extension directories. Built-in `APP/` source is intentionally excluded
from default assistant workspaces to avoid skill/context conflicts. See
...
- APP plugins, side-loaded extensions, extension commands, plugin manifests, and
  plugin-owned service scripts belong to `APP/[plugin]/` in source mode or
  `~/Documents/Galois/extensions/[plugin]/` in installed/user-extension mode.
...
- Extension `plugin.json` id: stable kebab-case or lower camelCase; do not
  reuse a notebook command id.
...
- Notebook command id: project-scoped id such as `project.noteStats` or
  `custom.insertStatus`; keep it separate from extension command ids.
...
User-developed extensions live under `~/Documents/Galois/extensions/`. This
...
The extension host refreshes side-loaded/user extension manifests while the app
is running and publishes the current extension and command lists into Blood.
Changes to `plugin.json` are picked up by the next refresh cycle. Changes to
service scripts are picked up on the next command execution because the script
path is resolved and executed from disk. Creating a new renderer UI panel still
requires source `APP/*/index.ts` development with Vite HMR, or a future dynamic
UI bundle loader.
