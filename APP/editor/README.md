...
- Live Preview 的媒体正文（删除按钮和播放器控件除外）在主键按下时把 CodeMirror 光标定位到 token 头部并撤销 widget；拖拽选区的任一端点刚接触媒体 Markdown 边界也视为已选中，因此从上向下或从下向上选择媒体都会立即显示原始 Markdown，无需越过整个媒体
...
- Reading 的删除叉号属于每个图片、音频、视频或时间线媒体实例，只删除被点击的完整 Markdown token；同一行的文字和其他媒体保留，只有删除后整行为空时才移除空行
...
- Live 与 Reading 粘贴后都会确保光标或目标区块处于可视区域；只有插入尾部越出编辑器可视框时才滚动，短内容仍完整可见时保持原滚动位置
- Reading 中无论粘贴的是纯媒体 Markdown、文字与媒体混合的 Markdown，还是剪贴板中的真实图片文件，只要载荷包含媒体 token，写入或异步归档完成后都会立即结束局部 textarea 编辑并失焦、重建全部文字和媒体块；区块生成及图片加载后会重新检查尾部，仅在内容被撑出可视框时滚动到插入尾部
...
- Reading 中同一 Markdown 行的图片使用等分响应式媒体列：同行图片获得相同可用列宽，容器不足时自动换行；换行后的单图扩展到该行宽度，图片始终保持原始纵横比
